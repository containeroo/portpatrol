...
func TestValidateHTTPChecker(t *testing.T) {
	t.Parallel()

	t.Run("valid HTTP URL", func(t *testing.T) {
		t.Parallel()

		name, err := validateHTTPChecker("example", "http://example.com")
		if err != nil {
			t.Fatalf("expected no error, got %v", err)
		}
		if name != "example" {
			t.Fatalf("expected name to be 'example', got %v", name)
		}
	})
	...
	t.Run("valid HTTPS URL", func(t *testing.T) {
		...
		name, err := validateHTTPChecker("", "https://example.com")
		if err != nil {
			t.Fatalf("expected no error, got %v", err)
		}
		if name != "example.com" {
			t.Fatalf("expected name to be 'example.com', got %v", name)
		}
	})
	...
	t.Run("missing scheme", func(t *testing.T) {
		...
		_, err := validateHTTPChecker("example", "example.com")
		if err == nil {
			t.Fatal("expected an error, got none")
		}
	})
	...
	t.Run("unsupported scheme", func(t *testing.T) {
		...
		_, err := validateHTTPChecker("example", "ftp://example.com")
		if err == nil {
			t.Fatal("expected an error, got none")
		}
	})
...
func TestValidateTCPChecker(t *testing.T) {
	t.Parallel()

	t.Run("valid TCP address with name", func(t *testing.T) {
		t.Parallel()

		name, err := validateTCPChecker("example", "example.com:80")
		if err != nil {
			t.Fatalf("expected no error, got %v", err)
		}
		if name != "example" {
	...
	t.Run("valid TCP address without name", func(t *testing.T) {
		t.Parallel()

		name, err := validateTCPChecker("", "example.com:80")
		if err != nil {
			t.Fatalf("expected no error, got %v", err)
		}
		if name != "example.com" {
	...
	t.Run("missing port", func(t *testing.T) {
		...
		_, err := validateTCPChecker("example", "example.com")
		if err == nil {
			t.Fatal("expected an error, got none")
		}
	})
